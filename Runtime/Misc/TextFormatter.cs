...
namespace StormiumTeam.ThirdParty
{
	...
	public unsafe class Converter : IConverter<int>, IConverter<float>, IConverter<string>, IConverter<byte>
	{
		...
		void IConverter<float>.Convert(ref char* dst, char* end, float value, FormatSpec formatSpec)
		{
			...
			var   v1        = Mathf.FloorToInt(value);
			float fractMult = (int) Mathf.Pow(10.0f, formatSpec.fractWidth);
			var   v2        = Mathf.FloorToInt(value * fractMult) % (int) fractMult;
		...
		private void ConvertInt(ref char* dst, char* end, int value, int argWidth, int integerWidth, bool leadingZero)
		{
    ...
    public static unsafe class StringFormatter
	{
		...
		private static int ReadNum(ref char* p)
		{
			var res = 0;
			var neg = false;
			if (*p == '-')
		...
		private static int CountChar(ref char* p, char ch)
		{
			var res = 0;
			while (*p == ch)
			{
